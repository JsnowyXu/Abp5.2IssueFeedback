...
namespace Fun2RepairMVC.Tests
{
    public abstract class Fun2RepairMVCTestBase : AbpIntegratedTestBase<Fun2RepairMVCTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected Fun2RepairMVCTestBase()
        {
        ...
        protected void UsingDbContext(Action<Fun2RepairMVCDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Fun2RepairMVCDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Fun2RepairMVCDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Fun2RepairMVCDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Fun2RepairMVCDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Fun2RepairMVCDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Fun2RepairMVCDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Fun2RepairMVCDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Fun2RepairMVCDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Fun2RepairMVCDbContext, Task<T>> func)
        {
            T result;
